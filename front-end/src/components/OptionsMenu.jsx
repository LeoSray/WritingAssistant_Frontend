    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button onClick={toggleDropdown} 
                ...
                data-dropdown-toggle="dropdown" className="relative focus:outline-none 
                font-medium rounded-2xl px-5 py-2.5 border border-gray-700 text-center inline-flex items-center
                w-auto sm:min-w-[200px] md:min-w-[400px] md:text-md lg:min-w-[550px]
            ...
            <div className={`z-10 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'} transition ease-in-out duration-300 absolute rounded-xl bg-white divide-y divide-gray-100 shadow w-full dark:bg-gray-800`}>
                <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                    ...
                    <li key={index}>
                        <a onClick={() => setValue(column)} href="#" className="block px-4 py-2 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 dark:hover:text-white">{column}</a>
                    </li>
                    ))}
                </ul>
            </div>
